...
from core.yolov4.utils import load_class_names, plot_boxes_cv2
import torch
import cv2
import numpy as np
...
def load_model():
    ...
    model.load_state_dict(torch.load('core/yolov4/yolov4.pt', map_location=device, weights_only=False))
    model.eval()
    return model
...
def plot_boxes(boxes, img):
    ...
    for i in range(len(boxes)):
        ...
        x2 = int(box[2] * width)
        y2 = int(box[3] * height)
        img = cv2.rectangle(img, (x1, y1), (x2, y2), color=class_colors[int(box[6])], thickness=2)
    
    return img

def inference_image(img: cv2.Mat):
    model = load_model()
    boxes = predict(model, img)
